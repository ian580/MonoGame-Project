...
namespace GameProject
{
    ...
    public class Game1 : Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
                ...
                for (int j = i + 1; j < bears.Count; j++)
                {
                    if (bears[i].Active && bears[j].Active)
                    {
                        ...
                        if (collision != null)
                        {
                            ...
                            {
                                // Teddy1 was forced out of bounds so remove from the game
                            ...
                            {
                                // Teddy2 was forced out of bounds so remove from the game
            ...
            {
                // Only check for collisions if this teddy is active
                if (teddy.Active)
                {
                    foreach (Projectile projectile in projectiles)
                    {
                        if (projectile.Type == ProjectileType.FrenchFries && projectile.Active)
                        {
                            ...
                            {
                                // Set the teddy and projectile to inactive
                                teddy.Active = false;
